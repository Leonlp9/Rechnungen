...
pub unsafe fn did_finish_launching() {
  ...
  for window in windows {
    ...
    if count > 1 {
      ...
      // ```
      //
      // NOTE: this `setScreen:` dance is only valid for legacy, non-scene windows.
      // A window created with `-initWithWindowScene:` derives its screen from its
      // scene, and forcing `.screen` directly on it leaves the window detached from
      // any scene/screen (a fully black, undrawn window) without raising any error.
      // Skip the dance for scene-backed windows; the scene handshake already
      // guarantees correct placement, so only the rootViewController reset (needed
      // for orientation) and `makeKeyAndVisible` are still useful.
