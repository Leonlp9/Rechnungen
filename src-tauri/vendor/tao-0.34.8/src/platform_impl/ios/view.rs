...
use objc2::runtime::{
  AnyClass as Class, AnyObject as Object, AnyProtocol, ClassBuilder as ClassDecl, Sel,
};
...
use crate::{
  ...
  platform_impl::platform::{
    ...
    ffi::{
      id, nil, CGFloat, CGPoint, CGRect, NSStringRust, UIForceTouchCapability,
      UIInterfaceOrientationMask, UIRectEdge, UITouchPhase, UITouchType, BOOL, NO, YES,
    },
...
// Set by `scene:willConnectToSession:options:` once UIKit hands us the
// connected `UIWindowScene` for this (single-scene) app. Read by
// `create_window` so new windows are attached to it via `initWithWindowScene:`,
// which is required for scene-lifecycle adoption on newer iOS versions.
static mut CONNECTED_WINDOW_SCENE: id = std::ptr::null_mut();
static mut DID_START_LAUNCHING: bool = false;

extern "C" {
  fn syslog(priority: std::os::raw::c_int, format: *const c_char, ...);
}

// TEMP DEBUG: traces the scene-lifecycle patch via the system log, so it can
// be read back with `idevicesyslog` / `idevicecrashreport` from the host.
fn dbg_log(msg: &str) {
  if let Ok(c) = CString::new(msg) {
    unsafe {
      syslog(5, b"TAO_SCENE_DBG: %s\0".as_ptr() as *const c_char, c.as_ptr());
    }
  }
}
...
pub unsafe fn create_window(
  ...
  dbg_log(&format!(
    "create_window: CONNECTED_WINDOW_SCENE is_null={}",
    CONNECTED_WINDOW_SCENE.is_null()
  ));
  let window: id = msg_send![class, alloc];
  assert!(!window.is_null(), "Failed to create `UIWindow` instance");
  let window: id = if CONNECTED_WINDOW_SCENE.is_null() {
    dbg_log("create_window: using initWithFrame (no scene)");
    msg_send![window, initWithFrame: frame]
  } else {
    dbg_log("create_window: using initWithWindowScene");
    let window: id = msg_send![window, initWithWindowScene: CONNECTED_WINDOW_SCENE];
    let () = msg_send![window, setFrame: frame];
    window
  };
  assert!(
    !window.is_null(),
...
pub fn create_delegate_class() {
  extern "C" fn did_finish_launching(_: &Object, _: Sel, _: id, _: id) -> BOOL {
    // Don't start dispatching tao events (and thus creating the app's window)
    // here yet: on scene-lifecycle iOS, `UIWindowScene` connection only begins
    // once this method returns, so any window tao creates in response would
    // still be scene-less and would never be composited on screen. Real event
    // dispatch is instead kicked off from `scene:willConnectToSession:options:`,
    // once a scene actually exists to attach the window to.
    YES
  }
  ...
  // iOS now requires apps to adopt the UIScene lifecycle; without a delegate
  // implementing `application:configurationForConnectingSceneSession:options:`
  // the OS refuses to create a scene for the app and terminates it on launch.
  // We reuse `AppDelegate` itself as the scene delegate rather than declaring
  // a separate Objective-C class.
  extern "C" fn configuration_for_connecting_scene_session(
    ...
    dbg_log("configuration_for_connecting_scene_session called");
    unsafe {
      let role: id = msg_send![connecting_scene_session, role];
      let config: id = msg_send![class!(UISceneConfiguration), alloc];
      let name = NSStringRust::alloc(nil).init_str("Default Configuration");
      let config: id = msg_send![config, initWithName: name sessionRole: role];
      let app_delegate_class = class!(AppDelegate);
      let _: () = msg_send![config, setDelegateClass: app_delegate_class];
      dbg_log("configuration_for_connecting_scene_session returning config");
      config
    }
  }
  ...
  extern "C" fn scene_will_connect_to_session(
    ...
    dbg_log("scene_will_connect_to_session called");
    unsafe {
      let is_window_scene: bool = msg_send![scene, isKindOfClass: class!(UIWindowScene)];
      dbg_log(&format!("scene_will_connect_to_session isWindowScene={is_window_scene}"));
      if is_window_scene {
        CONNECTED_WINDOW_SCENE = scene;
      }
      // Now that a scene is available, actually start dispatching tao events
      // (deferred from `application:didFinishLaunchingWithOptions:` - see the
      // comment there). This is what triggers the app to create its window,
      // and it will now correctly attach it via `initWithWindowScene:`.
      if !DID_START_LAUNCHING {
        DID_START_LAUNCHING = true;
  ...
  unsafe {
    ...
    decl.add_method(
      ...
      scene_will_connect_to_session as extern "C" fn(_, _, _, _, _),
    );

    if let Some(proto) = AnyProtocol::get(CStr::from_bytes_with_nul(b"UIWindowSceneDelegate\0").unwrap()) {
      decl.add_protocol(proto);
    }
    if let Some(proto) = AnyProtocol::get(CStr::from_bytes_with_nul(b"UISceneDelegate\0").unwrap()) {
      decl.add_protocol(proto);
    }
    if let Some(proto) =
      AnyProtocol::get(CStr::from_bytes_with_nul(b"UIApplicationDelegate\0").unwrap())
    {
      decl.add_protocol(proto);
    }
